...
!CT SEDNA Bug correction from CR start { 
            !qns(ji,jj) = qt_oce_ai(ji,jj) - zqsr              
            qns(ji,jj) = qt_oce_ai(ji,jj) - qsr(ji,jj)
!CT SEDNA Bug correction from CR end } 
...
!CT for SEDNA {
      IF( iom_use('qla_oce'    ) )   CALL iom_put( "qla_oce"    , qla_oce * ( 1._wp - at_i_b )                               )   !     latent   heat flux at ocean surface
      IF( iom_use('qsb_oce'    ) )   CALL iom_put( "qsb_oce"    , qsb_oce * ( 1._wp - at_i_b )                               )   !     sensible heat flux at ocean surface
      IF( iom_use('qlw_oce'    ) )   CALL iom_put( "qlw_oce"    , qlw_oce * ( 1._wp - at_i_b )                               )   !     longwave heat flux at ocean surface
!CT for SEDNA }
